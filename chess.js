...
let closemodal = document.querySelector('.close-btn').addEventListener('click', closeModal);
...
class Piece {
  ...
  check(arr, x, y) {
    ...
    if (this.name == "bishop") {
      ...
      let kai=[]
      let kdi=[]
      for(let i=0; i<panwdeksia.length; i++){
        if(panwdeksia[i].length==3 && this.color==panwdeksia[i][2].color){
          break;
        }
        
        if(panwdeksia[i].length==3 && this.color!=panwdeksia[i][2].color){
          pdi.push(panwdeksia[i]);
          break;
      }
          pdi.push(panwdeksia[i]);
      }
      for(let i=0; i<panwaristera.length; i++){
        if(panwaristera[i].length==3 && this.color==panwaristera[i][2].color){
          break;
        }
        if(panwaristera[i].length==3 && this.color!=panwaristera[i][2].color){
          pai.push(panwaristera[i]);
          break;
        }
        pai.push(panwaristera[i]);
      }
      for(let i=0; i<katwaristera.length; i++){
        if(katwaristera[i].length==3 && this.color==katwaristera[i][2].color){
          break;
        }
        if(katwaristera[i].length==3 && this.color!=katwaristera[i][2].color){
          kai.push(katwaristera[i]);
          break;
        }
        kai.push(katwaristera[i]);
      }
      for(let i=0; i<katwdeksia.length; i++){
        if(katwdeksia[i].length==3 && this.color==katwdeksia[i][2].color){
          break;
        }
        if(katwdeksia[i].length==3 && this.color!=katwdeksia[i][2].color){
        ...
        kdi.push(katwdeksia[i]);
         ...
         let  hl=[...pai, ...pdi, ...kai, ...kdi];
            ...
            let yAxb = parseInt(blackKing.substring(blackKing.length-2, blackKing.length-1));
            let whiteKing = localStorage.getItem('kingWhite');
            let xAxw = whiteKing.substring(whiteKing.length-4, whiteKing.length-3);
            let yAxw = parseInt(whiteKing.substring(whiteKing.length-2, whiteKing.length-1));
            if(this.color=='white'){
            for(let i=0; i<hl.length; i++){
             if(hl[i].slice(0,2).toString()==[xAxb, yAxb].toString()){
              return true;
              }
            }
          }else if(this.color=='black'){
            for(let i=0; i<hl.length; i++){
             if(hl[i].slice(0,2).toString()==[xAxw,yAxw].toString()){
              return true;
              }
            }
          }
...
for (let l = 0; l < arrx.length; l++) {
  ...
  if (
    xa.indexOf(xAx)<xa.indexOf(arrx[l].slice(0,1).toString())
  ) {
...
for(let i=0; i<d.length; i++){
  if(d[i].length==3 && this.color==d[i][2].color){
    break;
  }
...
for(let i=0; i<p.length; i++){
  if(p[i].length==3 && this.color==p[i][2].color){
    break;
  }
...
for(let i=0; i<a.length; i++){
  if(a[i].length==3 && this.color==a[i][2].color){
    break;
  }
...
for(let i=0; i<k.length; i++){
  if(k[i].length==3 && this.color==k[i][2].color){
    break;
  }
    ...
    if (this.name == "knight") {
      ...
      if (x == xAx || y == yAx) {
        ...
        if (kouti[0].length > 2) {
          let obj = kouti[0];
          let c = obj[2];
          if (c.color != this.color && c.name == "king") {
            return true;
          } else return false;
        }
      }
    ...
    if ((y == yAx && x != xAx) || (y != yAx && x == xAx)) {
      ...
      if (xa.indexOf(xAx) < xa.indexOf(x)) {
        ...
        if (!nearest) {
          ...
          xa.indexOf(nearest.slice(0, 1).toString()) >= xa.indexOf(x.toString())
        ) {
          return true;
        } else if (
          xa.indexOf(nearest.slice(0, 1).toString()) <= xa.indexOf(x.toString())
    ...
    if ((y == yAx && x != xAx) || (y != yAx && x == xAx)) {
      if (yAx < y && x == xAx) {
        let possibleMove = [];
          ...
          for (let l = 0; l < arra.length; l++) {
            if (parseInt(arra[l].slice(1, 2)) <= y && parseInt(arra[l].slice(1, 2)) > yAx) {
              possibleMove.push(arra[l]);
            } else {
              break;
            }
      ...
      if (ioyf < ioy && ioxf < iox) {
        ...
        let nearest = panwdeksia.find((x) => x.length == 3 && x[2].name != "king");
  ...
  highLightMoves(arr) {
    ...
    if(this.name=='pawn'&& this.color=='white'){
      for(let i=0; i<arr.length; i++){
        ...
        (arr[i].slice(0,1).toString()==xa[xa.indexOf(xAx)+1] && parseInt(arr[i].slice(1,2))==yAx+1 && arr[i].length==3 && f[2].color!=this.color)||
        (arr[i].slice(0,1).toString()==xa[xa.indexOf(xAx)-1] && parseInt(arr[i].slice(1,2))==yAx+1 && arr[i].length==3 && f[2].color!=this.color)){
     
          let div = document.getElementById(arr[i].slice(0,2).join("").toString());
          
          div.style.backgroundColor = "#965bf5";
        }else if(yAx!=2 && 
  ...
  if(k[i].length==3 && this.color!=k[i][2].color){
    ...
    ki.push(k[i]);   
     ...
     hl=[...pi, ...di, ...ki, ...ai];
      ...
      for(let i=0; i<hl.length; i++){
        let div = document.getElementById(hl[i].slice(0,2).join("").toString());
        div.style.backgroundColor = "#965bf5";
      }
    }
    if(this.name=='bishop'){
...
for(let i=0; i<k.length; i++){
  ...
  if(k[i].length==3 && this.color!=k[i][2].color){
    ...
    ki.push(k[i]);   
         ...
         let  hl=[...pai, ...pdi, ...kai, ...kdi,...pi, ...di, ...ki, ...ai];
    ...
    if(this.name=='knight'){
      ...
      for(let i = 0; i<arr.length; i++){
       ...
       if (
        (arr[i].slice(0,1).toString() == xa[ioxf + 1] && parseInt(arr[i].slice(1,2)) == ya[ioyf + 2]) ||
        (arr[i].slice(0,1).toString() == xa[ioxf + 2] && parseInt(arr[i].slice(1,2)) == ya[ioyf + 1]) ||
        (arr[i].slice(0,1).toString() == xa[ioxf + 1] && parseInt(arr[i].slice(1,2)) == ya[ioyf - 2]) ||
        (arr[i].slice(0,1).toString() == xa[ioxf - 2] && parseInt(arr[i].slice(1,2)) == ya[ioyf + 1]) ||
        (arr[i].slice(0,1).toString() == xa[ioxf - 1] && parseInt(arr[i].slice(1,2)) == ya[ioyf + 2]) ||
        (arr[i].slice(0,1).toString() == xa[ioxf + 2] && parseInt(arr[i].slice(1,2)) == ya[ioyf - 1]) ||
       ...
       if(arr[i].length==2){
        pm.push(arr[i]);
      }else if(arr[i].length==3 && arr[i][2].color!=this.color){
        pm.push(arr[i]);
      }
      }
      }
      for(let i=0; i<pm.length; i++){
        let div = document.getElementById(pm[i].slice(0,2).join("").toString());
        div.style.backgroundColor = "#965bf5";
      }
    }
  }
  ...
  Array.from(u).forEach((button) =>
    button.addEventListener("click", (event) => {
      ...
      if (!firstClickTarget) {
        ...
        if(firstClickTarget.name == "king"){
          if(firstClickTarget.color == "white"){
            let isValid =[]
            let pm = Piece.whiteKingPossibleMoves(board);
            
            for(let i=0; i<pm.length; i++){
              
              if(pm[i].slice(0,2).join("").toString() == button.value.toString()){
                  isValid.push(pm[i]);
              }
            }
           console.log("Is valid: ", isValid);
            ...
            if(firstClickTarget.color == "black"){
              let isValid =[]
              let pm = Piece.blackKingPossibleMoves(board);
        ...
        if (turns % 2 == 0 && shouldBlackKingMove ) {
          ...
          for(let i=0;i<newArr1.length;i++){
            ...
            for (let j = 0; j < gg.length; j++) {
             ...
             if(resultA){
          thirdloop: for (let l = 0; l < newArr2.length; l++) {
              if (
                resultA.length == 64 &&
                newArr2[l].name != "king" &&
          ...
          console.log("Check count: ", checkCount);
          if(checkCount ==0 && Piece.blackKingPossibleMoves(moveResult).length == 0){
        ...
        if (turns % 2 != 0 && shouldWhiteKingMove) {
          ...
          console.log("Check count: ", checkCount);
          if(checkCount ==0 && Piece.whiteKingPossibleMoves(moveResult).length == 0){
            console.log("game has ended");
            return
        ...
        if ( shouldBlackKingMove) {
          ...
          for(let i=0;i<newArr1.length;i++){
            for(let j=0;j<after.length;j++){
             const resultA = aBoard(newArr1[i],after[j].slice(0,1).toString(),parseInt(after[j].slice(1,2)));
             let gg=[];
             let newArr2 = [];
             for (let i = 0; i < resultA.length; i++) {
              if (resultA[i].length > 2) {
          ...
          console.log("Check count: ", checkCount);
          if(checkCount ==0 && Piece.blackKingPossibleMoves(after).length == 0){
        ...
        if(turns % 2 != 0 && shouldBlackKingMove==false && Piece.blackKingPossibleMoves(board).length == 0){
          let moveCount = 0;
          for (let i = 0; i < after.length; i++) {
            if (after[i].length > 2) {
              let g =after[i];
          ...
          if(moveCount == 0){
            console.log("game has ended, its a stalemate");
            draw= true;
        ...
        if(turns % 2 == 0 && shouldWhiteKingMove==false && Piece.whiteKingPossibleMoves(board).length == 0){
          let moveCount = 0;
          for (let i = 0; i < after.length; i++) {
            if (after[i].length > 2) {
              let g = after[i];
...
document.getElementById("turns").innerText =
  localStorage.getItem("turns")
    ...
    if(localStorage.getItem("isOver") == "true"){
      afterMate();
    }
    if(localStorage.getItem("draw") == "true"){
      const whowon= document.getElementById("whoWon");
      whowon.innerText = "Game ended in a draw";
      openModal();
    }
